...
In Cocoa, methods that produce errors take an [NSError](/documentation/Foundation/NSError) pointer parameter as their last parameter, which populates its argument with an [NSError](/documentation/Foundation/NSError) object if an error occurs. Swift automatically translates Objective-C methods that produce errors into methods that throw an error according to Swift’s native error handling functionality.
...
For example, consider the [removeItem(at:)](/documentation/Foundation/FileManager/removeItem(at:)) method from [FileManager](/documentation/Foundation/FileManager). In Objective-C, it’s declared like this:
...
Notice that the [removeItem(at:)](/documentation/Foundation/FileManager/removeItem(at:)) method is imported by Swift with a `Void` return type, no error parameter, and a `throws` declaration.
