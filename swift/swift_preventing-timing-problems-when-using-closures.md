...
A common timing mistake is expecting the results of an asynchronous call to be available within the calling synchronous code. For example, the `later(_:)` method above is comparable to the [URLSession](/documentation/Foundation/URLSession) class’s [dataTask(with:completionHandler:)](/documentation/Foundation/URLSession/dataTask(with:completionHandler:)-52wk8) method, which is also asynchronous. A timing scenario you should avoid is calling the [dataTask(with:completionHandler:)](/documentation/Foundation/URLSession/dataTask(with:completionHandler:)-52wk8) method within your app’s [viewDidLoad()](/documentation/UIKit/UIViewController/viewDidLoad()) method and attempting to use the results outside of the closure you pass as the completion handler.
...
The example below creates a [FileHandle](/documentation/Foundation/FileHandle) and an array of data lines to write to the file that the handle refers to:
...
To write each line to the file, pass a closure to the [forEach(_:)](/documentation/swift/array/foreach(_:)) method:
...
When you’re finished using a [FileHandle](/documentation/Foundation/FileHandle), close it using [closeFile()](/documentation/Foundation/FileHandle/closeFile()). The correct placement of the call to [closeFile()](/documentation/Foundation/FileHandle/closeFile()) is outside of the closure:
...
If you misunderstand the requirements of [closeFile()](/documentation/Foundation/FileHandle/closeFile()), you might place the call inside the closure. Doing so crashes your app:
