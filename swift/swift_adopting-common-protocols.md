...
`NSObject` subclasses inherit conformance to the `Equatable` and `Hashable` protocols, with equality based on instance identity. If you need to customize this behavior, override the [isEqual(_:)](/documentation/ObjectiveC/NSObjectProtocol/isEqual(_:)) method and [hash](/documentation/ObjectiveC/NSObjectProtocol/hash) property instead of the `==` operator method and `hashValue` property.
