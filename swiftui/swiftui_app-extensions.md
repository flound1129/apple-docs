...
Use SwiftUI along with [WidgetKit](/documentation/WidgetKit) to add widgets to your app.
...
- [accessoryWidgetGroupStyle(_:)](/documentation/swiftui/view/accessorywidgetgroupstyle(_:)): The view modifier that can be applied to `AccessoryWidgetGroup` to specify the shape the three content views will be masked with. The value of `style` is set to `.automatic`, which is `.circular` by default.
