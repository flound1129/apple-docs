...
To display read-only text, or read-only text paired with an image, use the built-in [Text](/documentation/swiftui/text) or [Label](/documentation/swiftui/label) views, respectively. When you need to collect text input from the user, use an appropriate text input view, like [TextField](/documentation/swiftui/textfield) or [TextEditor](/documentation/swiftui/texteditor).
